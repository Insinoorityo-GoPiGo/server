...
def start_thread(client_api):
    ...
    (threading.Thread(target=run_server, args=(client_api,), daemon=False)).start()
...
def main():
    ...
    client_api_1 = ClientAPI(host="192.168.34.87", port=1025, path=path1, quit_flag=quit_flag, location_queue=location_queue, default_direction="east", bot_id="Bot_1")
    client_api_2 = ClientAPI(host="192.168.34.87", port=1026, path=path2, quit_flag=quit_flag, location_queue=location_queue, default_direction="west", bot_id="Bot_2")
